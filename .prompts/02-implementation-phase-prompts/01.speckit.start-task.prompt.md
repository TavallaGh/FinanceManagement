...
3. Load required references:
   ...
   - **Required for Jira/GitLab flow tasks**: `.prompts/02-implementation-phase-prompts/00.speckit.taskstoissues.prompt.md`
...
5. Classify task type and choose documentation output:
   ...
   - If task is **not** a Domain Design task:
     - Generate task implementation plan markdown only (no code) and ensure it is production-ready:
       - Use `docs/work-items/02.implementation/templates/task-record.template.md` as the structural base.
...
8. Post-approval handoff behavior:
   ...
   - Instruct user to run `/speckit.implement` with the approved plan path.
