---
description: Execute the TL-approved implementation plan and deliver production-ready code in the referenced repository targets using TDD-first and BDD validation.
---
...
2. Resolve implementation context from input:
   ...
   - Resolve the authoritative task and approved implementation plan paths under `docs/work-items/02.implementation/**`:
     - For story subtasks: `docs/work-items/02.implementation/stories/<PARENT-STORY-KEY>/tasks/<TASK-KEY>-implementation-plan.md`
     - For standalone tasks: `docs/work-items/02.implementation/standalone/<type>/<TASK-KEY>-implementation-plan.md`
...
3. Load required references before coding:
   ...
   - If Jira/GitLab flow is required, load `.prompts/02-implementation-phase-prompts/00.speckit.taskstoissues.prompt.md`.
...
12. Completion output:
   ...
   - Do NOT run `task-exec.ps1`, do NOT transition Jira, do NOT mark MRs as Ready.
   - Remind the user to run `/speckit.taskcompletion` when implementation finished.
...
- **NEVER** call `task-exec.ps1`, transition Jira status, or mark GitLab MRs as Ready during implementation. Those operations belong exclusively to `/speckit.taskclose`.
- Implementation scope ends when: code is written, tests pass, and changes are committed to the feature branch. Nothing more.
