...
- For Jira subtask create/update use `scripts/speckit-tasks.ps1` (wrapper) and `scripts/create-jira-subtask.ps1` (core reusable executor).
...
### Step 5 — Create Jira Subtasks (Import Task Descriptions)

For each task in the task plan, create a Jira subtask under the parent story.

**Execution method (required):** use reusable script `scripts/speckit-tasks.ps1`.

For each generated task detail file `docs/work-items/01.solution/linked/stories/{STORY_KEY}/tasks/{TASK_SLUG}.md`, call:

```powershell
./scripts/speckit-tasks.ps1 \
  -Summary "{TASK_NAME}" \
  -ParentKey "{STORY_KEY}" \
  -TaskFile "docs/work-items/01.solution/linked/stories/{STORY_KEY}/tasks/{TASK_SLUG}.md" \
  -Labels @("backend")
```

The script must import the task markdown file content as Jira issue description (ADF) so the created Jira subtask contains the full task description.
...
- AoC, DoD, Goal, Problem, and full task description content must be imported from each task detail file.
