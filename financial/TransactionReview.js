  ...
  const TransactionReview = ({ language = 'fa', formCode = 'FIN_TRANSACTION_REVIEW' }) => {
    ...
    useEffect(() => {
        const handleFilterToRecord = (e) => {
            if (e.detail && e.detail.form_component === 'TransactionReview') {
    ...
    const viewConfig = useMemo(() => ({
      pageId: 'transactions_review_list',
    ...
    const DetailsModal = safeComp(window, 'TransactionRevviewDetails');
    ...
    return React.createElement('div', { className: "p-4 h-full flex flex-col font-sans bg-slate-50/50 dark:bg-slate-900", dir: isRtl ? 'rtl' : 'ltr' },
        ...
        CommentModal && commentModalState.isOpen ? React.createElement(CommentModal, {
            ...
            formComponent: 'TransactionReview',
            language: language
        }) : null,
