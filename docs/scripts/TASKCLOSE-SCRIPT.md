...
The script automatically detects and documents new API endpoints in backend repositories.

### How It Works

**Scan-BackendEndpoints Function:**
1. Scans backend repositories for new/modified endpoints:
   - `projects/accounting-sso` (primary)
   - `projects/Accounting-Project` (fallback)
   - `projects/accounting-prototype` (fallback)

2. Detects C# controller decorators:
   - `[HttpGet(...)]`
   - `[HttpPost(...)]`
   - `[HttpPut(...)]`
   - `[HttpDelete(...)]`
...
3. Detects Minimal API patterns:
   - `.MapGet(path, handler)`
...
4. Extracts route attributes:
   - `[Route(...)]`
   - `[HttpRoute(...)]`

### Generate-Postman Integration

When endpoints are detected, the Postman collection includes:
- **API Methods & Paths** - All HTTP methods and endpoint routes
- **Request Bodies** - Schema examples for POST/PUT/PATCH
- **Response Schemas** - Expected response formats
- **Authentication** - Bearer tokens, API keys, headers
- **Environment Variables**:
  - `base_url`: API base URL (http://localhost:5000, etc.)
  - `auth_token`: Authorization token
  - `environment`: Dev/Test/Prod selector

### CHANGELOG Generation

Endpoints are documented in CHANGELOG.md with:
- **Endpoint Table** - Method, Path, Auth requirements
- **Request/Response Schemas** - JSON structure examples
- **Authentication Requirements** - Bearer token, API key, etc.
- **Rate Limiting** - Quotas and limits if applicable
- **Breaking Changes** - Migration guide for API changes

### Example Output

**Detected Endpoints:**
```
[GET]    /api/accounts               (AccountsController)
[POST]   /api/accounts               (AccountsController)
[GET]    /api/transactions           (TransactionsController)
[POST]   /api/transactions           (TransactionsController)
[DELETE] /api/transactions/{id}      (TransactionsController)
```

**Postman Collection:**
```json
{
  "info": { "name": "Task API Endpoints", "version": "1.0" },
  "item": [
    {
      "name": "Accounts - List All",
      "request": {
        "method": "GET",
        "url": "{{base_url}}/api/accounts",
        "auth": { "type": "bearer", "bearer": "{{auth_token}}" }
      }
    }
  ]
}
