...
- **Automatically detects new endpoints:**
  - C# controller decorators: `[HttpGet]`, `[HttpPost]`, `[HttpPut]`, `[HttpDelete]`, `[HttpPatch]`
  - Minimal API patterns: `MapGet()`, `MapPost()`, `MapPut()`, `MapDelete()`, `MapPatch()`
  - Route attributes: `[Route(...)]`
- Generates postman.collection.json
- Includes environment variables
- Can be skipped with `-GeneratePostman $false`

### PHASE 4a: Backend Endpoint Detection
- **Scans backend repositories:**
  - `projects/accounting-sso`
  - `projects/Accounting-Project`
  - `projects/accounting-prototype`
- **Detects endpoint modifications** and new endpoints
- **Populates Postman collection** with actual endpoints (not generic stubs)
- **Includes in CHANGELOG.md:**
